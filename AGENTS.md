...
- **Skip-on-error**: a project source that fails to compile is dropped from `libproject.a` rather than failing the whole archive, so one broken WIP file (e.g. a module the project's own Makefile excludes) cannot block every test. The generated Makefile uses error-tolerant object rules (`-gcc … || rm -f $@`) and an archive recipe that **deletes the archive before recreating it** with only the objects that actually compiled. The leading `rm -f` is essential: `ar rcs` is additive and never removes members absent from its argument list, so without the delete a source that used to compile but now fails would leave its stale object in the archive — tests would link against the stale-but-working object and pass, silently masking the breakage. Recreating from scratch guarantees the archive reflects exactly what compiled, so a test whose dependency breaks now fails at link time (`undefined reference`, captured in `run.compile_err`) with the gcc compile error of the broken source visible in its log. Skipped sources are recorded in `RunnerState.skipped_sources` with their aggregated gcc stderr in `RunnerState.build_stderr` and **per-source** stderr in `RunnerState.source_errors` (all mirrored to global `state` via the legacy `runner/makefile.py` shim); headless prints a stderr warning with the compiler output, and the TUI shows them in the `#dep-warning` banner.
- **Per-test compile-error filtering**: when a test fails to build because a linked dependency source was skipped, its log shows only the compile errors of the sources *it* actually links — not every broken source in the project. `_assemble_compile_error` (in `api/_runner.py`) intersects `test.dependencies` (the linker-truth set from the prior `.map`) with `skipped_sources` and pulls each relevant source's isolated stderr from `source_errors`, plus the `undefined reference` link lines extracted from `make`'s stderr. So `test_interpreter` (which links only `interpreter.c`) shows `interpreter.c`'s gcc error + the link failure, never `expand.c`'s. Fallback: if the test has no recorded dependencies yet (first run, no `.map`) or none of its deps were skipped, the raw `make` stderr is shown verbatim (also covers the case where the test file *itself* has the compile error).
